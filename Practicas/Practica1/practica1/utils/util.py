...
def print_tabulate(df: pd.DataFrame) -> None:
    """
    Imprime un DataFrame en formato de tabla utilizando la función tabulate.

    Parámetros:
        df (pd.DataFrame): El DataFrame a imprimir.

    Retorna:
        None: Esta función no retorna ningún valor, solo imprime la tabla en la consola.

    Ejemplo:
        >>> import pandas as pd
        >>> df = pd.DataFrame({'Nombre': ['Alice', 'Bob', 'Charlie'], 'Edad': [25, 32, 22]})
        >>> print_tabulate(df)
        | Nombre  | Edad |
        |---------+------|
        | Alice   |   25 |
...
def print_salto_linea() -> None:
    print('\n')


def get_data(data_file: str = 'typed_uanl.csv') -> pd.DataFrame:
    df = pd.read_csv(f'csv/{data_file}')

    # Convertir la columna 'Fecha' a tipo datetime
    df[Columna.FECHA] = pd.to_datetime(df[Columna.FECHA], format="%Y-%m-%d")

    # Extraer el año de la columna 'Fecha'
    df[Columna.ANIO] = df[Columna.FECHA].dt.year
...
material_colors = [
    ...
    '#5D4037', '#4E342E', '#3E2723', '#FAFAFA', '#F5F5F5', '#EEEEEE', '#E0E0E0', 
    '#BDBDBD', '#9E9E9E', '#757575', '#616161', '#424242', '#212121', '#ECEFF1', 
